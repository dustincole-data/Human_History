...
- [04 — The scroll mechanic](issues/04-scroll-mechanic.md) — **partly settled, still open.** The fall is now a pure function of scroll: an arrival owns 1,000px, its fall spends 800 of them, height is `arc(t)` of the way down, and contact happens at t=1 exactly because the landing offset is solved from the real silhouette before the fall starts. **Three rulings.** (a) *matter.js is deleted entirely* — nothing was left worth keeping, and the 880ms floor, the queue and "drag to throw" went with it; one-at-a-time is now true by construction rather than by rate-limiting. (b) *The fall reverses, the break does not* — scrolling up mid-fall raises the object, but contact latches permanently and 13's one-way decay is untouched; the line is between a position and an event. (c) *The scroll is the only clock* — the wall clock was also in the shard scatter, the impact dust and the light's ease, so all three were converted; the frame is byte-identical after two idle seconds. Two live bugs fell out of the gates rather than the eye: a ghost sprite left hanging by a scrollbar jump, and 13's label clamp printing the name across the photograph on every arrival. **Still open in 04:** the prologue seam, ambient contemporaries, whether 235,100px total is too long — and **decoded memory measured at 301 MB in-page against an 80 MB gate**, now the project's biggest technical risk.
